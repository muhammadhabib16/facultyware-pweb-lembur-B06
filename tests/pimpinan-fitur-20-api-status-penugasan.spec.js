...
test('Pimpinan Fitur 20: REST API Status Penugasan', async ({ page }) => {
  await loginUser(page, PIMPINAN_EMAIL, PIMPINAN_PASS, 'Pimpinan');
  const apiPenugasanRes = await page.evaluate(async () => {
    const res = await fetch('/api/pimpinan/penugasan/status');
    return { status: res.status, data: await res.json() };
  });
  expect(apiPenugasanRes.status).toBe(200);
  expect(apiPenugasanRes.data.status).toBe('success');
});
