...
test('Pegawai Fitur 10: Pencarian Penugasan Lembur', async ({ page }, testInfo) => {
  const browserName = testInfo.project.name || 'browser';
  const uniqueTitle = `Cari Penugasan - ${browserName} - ${Date.now()}`;
  ...
  await page.fill('input[name="keyword"]', uniqueTitle);
