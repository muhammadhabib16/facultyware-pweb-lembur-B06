...
test('Pimpinan Fitur 29: Mengekspor Rekap Bulanan Semua Pegawai ke PDF', async ({ page }) => {
  ...
  const [downloadRekapPDF] = await Promise.all([
    page.waitForEvent('download'),
    page.click('text=Ekspor Rekap PDF', { force: true })
  ]);
  expect(downloadRekapPDF.suggestedFilename()).toContain('.pdf');
});
