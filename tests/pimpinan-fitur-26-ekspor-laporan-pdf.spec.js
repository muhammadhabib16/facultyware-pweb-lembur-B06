...
test('Pimpinan Fitur 26: Mengekspor Laporan Lembur ke PDF', async ({ page }, testInfo) => {
  const browserName = testInfo.project.name || 'browser';
  const uniqueTitle = `Pdf Lap - ${browserName} - ${Date.now()}`;
  ...
  await page.fill('textarea[name="notes"]', 'Laporan PDF.');
