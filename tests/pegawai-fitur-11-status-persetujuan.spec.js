...
test('Pegawai Fitur 11: Melihat Status Persetujuan Lembur', async ({ page }, testInfo) => {
  const browserName = testInfo.project.name || 'browser';
  const uniqueTitle = `Status Persetujuan - ${browserName} - ${Date.now()}`;
  ...
  await expect(page.locator('text=Assigned — Belum Dilaporkan')).toBeVisible();
});
