...
test('Pegawai Fitur 12: Mengekspor Detail Penugasan Lembur ke PDF', async ({ page }, testInfo) => {
  const browserName = testInfo.project.name || 'browser';
  const uniqueTitle = `Ekspor Detail PDF - ${browserName} - ${Date.now()}`;
  
  // Prasyarat: Pimpinan buat tugas
  ...
  await taskRow.getByRole('link', { name: 'Detail', exact: true }).click();
  const [downloadDetailPdf] = await Promise.all([
    page.waitForEvent('download'),
    page.click('text=Export PDF')
  ]);
  expect(downloadDetailPdf.suggestedFilename()).toContain('.pdf');
});
