...
test('Pimpinan Fitur 23: Melihat Detail Laporan Lembur', async ({ page }, testInfo) => {
  const browserName = testInfo.project.name || 'browser';
  const uniqueTitle = `Detail Lap Pimpinan - ${browserName} - ${Date.now()}`;
  ...
  await page.fill('textarea[name="notes"]', 'Laporan detail.');
