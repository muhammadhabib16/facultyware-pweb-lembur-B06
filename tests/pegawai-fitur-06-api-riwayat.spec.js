...
test('Pegawai Fitur 06: REST API Riwayat Lembur Pribadi', async ({ page }) => {
  test.setTimeout(60000); // Set timeout 1 menit
  await loginUser(page, PEGAWAI_EMAIL, PEGAWAI_PASS, 'Pegawai');
  const apiRiwayatRes = await page.evaluate(async () => {
    const res = await fetch('/api/pegawai/riwayat');
    return { status: res.status, data: await res.json() };
  });
  expect(apiRiwayatRes.status).toBe(200);
  expect(apiRiwayatRes.data.status).toBe('success');
});
