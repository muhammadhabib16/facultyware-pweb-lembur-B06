...
test('Pegawai Fitur 02: Mengajukan Laporan Realisasi Lembur', async ({ page }, testInfo) => {
  test.setTimeout(120000); // Set timeout 2 menit
  const browserName = testInfo.project.name || 'browser';
  const uniqueTitle = `Realisasi - ${browserName} - ${Date.now()}`;

  // Prasyarat: Pimpinan buat tugas (agar status = 'assigned')
  ...
  await page.fill('textarea[name="notes"]', 'Laporan Realisasi Kerja.');
